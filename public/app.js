...
function stopDancing() {
  if (!me.dancing) return
  me.dancing = false
  send({ t: 'dance', dancing: false })
  refreshRoster()
}
...
function drawAvatar(p, videoEl, isMe, inCall, now) {
  ...
  const bw = 12 * s
  const headR = 22
  const cx = p.x + (p.dancing && !reducedMotion.matches ? Math.sin(now / 1000 * 6) * 5 : 0)
