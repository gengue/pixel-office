...
test('office atlas loads before use and every furniture crop stays within the image', () => {
  ...
  try {
    const types = ['desk', 'chair', 'plant', 'shelf', 'ctable', 'ctableBig', 'fridge', 'counterH', 'sofaH', 'sofaV', 'tv', 'board', 'dtable', 'rug', 'armchair', 'planter', 'sideboard', 'water']
    const furniture = types.map((type) => [type, 100, 300, 150, 70])
    for (const [type, variant] of [['plant', 'snake'], ['plant', 'palm'], ['plant', 'rubber'], ['plant', 'fern'], ['plant', 'flowers'], ['sofaH', 'tealSofa'], ['chair', 'diningChair'], ['armchair', 'mustardChair'], ['ctable', 'bistro'], ['desk', 'laptopDesk']])
    ...
    for (const [source, x, y, w, h, dx, dy, dw, dh] of spriteDraws) {
      ...
      expect(x + w).toBeLessThanOrEqual(source === atlases[0] ? 1254 : 1448)
      expect(y + h).toBeLessThanOrEqual(source === atlases[0] ? 1254 : 1086)
