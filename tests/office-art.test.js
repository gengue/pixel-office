...
test('office atlas loads before use and every furniture crop stays within the image', () => {
  ...
  try {
    ...
    for (const [source, x, y, w, h, dx, dy, dw, dh] of spriteDraws) {
      ...
      expect(x + w).toBeLessThanOrEqual(source === atlases[0] ? 1254 : source === atlases[1] ? 1448 : 1536)
      expect(y + h).toBeLessThanOrEqual(source === atlases[0] ? 1254 : source === atlases[1] ? 1086 : 1024)
