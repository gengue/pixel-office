...
test('screen sharing is limited to its audience, one presenter per room, and stops on exit', async () => {
  ...
  try {
    ...
    // Meeting-room reactions stay with the audible audience inside.
    expect(outside.messages.filter((m) => m.t === 'reaction')).toHaveLength(0)
    ...
    const unknown = new WebSocket(`ws://localhost:${port}/ws`)
    clients.push(unknown)
    await wait(() => unknown.readyState === 1)
    unknown.send(JSON.stringify({ t: 'signal', to: owner.id, data: { kind: 'offer', sdp: 'unjoined' } }))
    outside.send({ t: 'signal', to: owner.id, data: { kind: 'offer', sdp: 'through-wall' } })
    outside.send({ t: 'move', x: 100, y: 1300 })
    outside.send({ t: 'signal', to: owner.id, data: { kind: 'offer', sdp: 'distant' } })
    viewer.send({ t: 'signal', to: owner.id, data: { kind: 'offer', sdp: 'room' } })
    await wait(() => owner.messages.some((m) => m.t === 'signal'))
    expect(owner.messages.filter((m) => m.t === 'signal').map((m) => m.data.sdp)).toEqual(['room'])
    outside.send({ t: 'move', x: 1760, y: 180 })
