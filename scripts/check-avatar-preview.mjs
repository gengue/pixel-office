...
try {
  ...
  assert.equal(await page.locator('#dance').getAttribute('aria-pressed'), 'true')
  assert(await page.locator('#direction').isDisabled(), 'Dance must lock direction')
  const dancing = await pixels()
  await page.waitForTimeout(270)
  assert.notEqual(await pixels(), dancing, 'Dance control must animate its own poses')
  await page.locator('#sit').click()
  await page.waitForTimeout(60)
  assert.equal(await page.locator('#dance').getAttribute('aria-pressed'), 'false')
  ...
  await page.locator('#dance').click()
  await page.waitForTimeout(80)
  const reducedDance = await pixels()
  await page.waitForTimeout(300)
  assert.equal(await pixels(), reducedDance, 'Reduced motion must keep dance static')
